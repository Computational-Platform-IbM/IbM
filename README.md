...
Function to create 2D substrates profiles for a given simulation number → `plot2dSubsProfile(simulation_number, substrates_number, Time)`.<br>
<sub>Inputs: `simulation_number` is the chosen simulation, `substrate_number` following order of Excel, and `Time` in days.</sub><br>
