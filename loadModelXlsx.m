...
route = char('A_S1.xlsx');% Reading the Excel file
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Parameters of the microbial species (umax, Ks, Yield)
...
aux = strcmp('Ks_A',SpecNames(1,2:end));
Ks_A = SpecParam((isfinite(SpecParam(:,aux))),aux);

pTh.Ks = [Ks_A];
% pTh.Ks = [Ks_A, Ks_B, Ks_O2];
...
aux = strcmp('maint',SpecNames(1,2:end));
...
xaux = ((1*10^(-7)+Sxy.T_blayer):(2*bac.bac_rmax):(Sxy.maxxSys-(1*10^(-7)+Sxy.T_blayer)));
yaux = ((1*10^(-7)+Sxy.T_blayer):(2*bac.bac_rmax):(Sxy.maxxSys-(1*10^(-7)+Sxy.T_blayer)));
...
% SS1 = zeros(4,length(Vlot)-1);
for i = 1: length(x_pn)
     if normd(i) < Vlot(end)
         for j = 1: length(Vlot)-1
             SSaux = floor(rand*St.numX)+1;
         end
        bac_s(end+1,1) = SSaux;
        bac_x(end+1,1) = x_pn(i);
        bac_y(end+1,1) = y_pn(i);
        norm_bac = sqrt((bac_x-bac.bac_c).*(bac_x-bac.bac_c)+(bac_y-bac.bac_c).*(bac_y-bac.bac_c));
     end
end
auxSize = size(bac_y);
...
% bac_s = kron(ones(round(auxSize(1)/St.numX), 1), (1:St.numX)') ;      % species attribute (types of cells)
...
bac.bac_ns = bac_ns;
bac.atrib = [bac_x, bac_y, bac_m, bac_a, bac_s, bac_r, bac_mu_max, bac_yield, bac_maint];

% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[DiffusionParam, ~] = xlsread(route, strcat('Diffusion'));
...
Sxy.pos_xySys = 0;
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[TolParam] = xlsread(route, strcat('Tolerances'));

kTr.Tolabs = TolParam(1,:);
kTr.Tolrel = TolParam(2:end,:);

% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Keq = pTh.Keq; chrM = pTh.chrM; spcR = pTh.spcR; w = 1; numStVLiq = St.numStVLiq;
