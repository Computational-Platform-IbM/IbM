...
%     axis equal; axis([minix maxix miniy maxiy]);
    axis equal; axis([200 800 700 900]); %um
...
%     Si = reshape(SSxy(:,j,:),R.Sxy.nySys-2,nT);
    ...
    S4 = reshape(SSxy(:,4,:)*(32/17),R.Sxy.nySys-2,nT);
    S5 = reshape(SSxy(:,5,:)*(44/17),R.Sxy.nySys-2,nT);
    S6 = reshape(SSxy(:,6,:)*(96/17),R.Sxy.nySys-2,nT);
    S7 = reshape(SSxy(:,7,:)*(70/17),R.Sxy.nySys-2,nT);
%     Smax = max(R.Sxy.Sbc_Dir(j),max(max(Si)));
    ...
    for i = 2:nT
        ...
        hold on  
        plot(minbac_x*1e6,R.Sxy.Sbc_Dir(1)*(17*1000), 'k*')
        hold on
        plot(maxbac_x*1e6,R.Sxy.Sbc_Dir(1)*(17*1000), 'k*')
        hold off      
...
%         hold on
        plot(minbac_x*1e6,R.Sxy.Sbc_Dir(4)*(32*1000), 'k*')
        hold on
        plot(maxbac_x*1e6,R.Sxy.Sbc_Dir(4)*(32*1000), 'k*')
        hold off
...
time = All_StatesVar(:,1);
    ...
    MW = [17*1000 17*1000 17*1000 32*1000 45*1000 96*1000];
    for i = 1: n_Liq
