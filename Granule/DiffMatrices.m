function [R, StVLiq, pH, Fb_layer, Bc, Lxy, CC, DD] = DiffMatrices(Bc, Lxy, R, StVLiq, pH, CC, DD, Fdiv)
    ...
    if Fdiv == 1
        ...
        v_theta = min(theta_c):d_theta:pi;
        ...
        for i = 1:R.Sxy.nT 
            ...
            % <E: I guess that you think this because in Line 15 v_theta is
            % from min(theta_c) to pi. It is important to know well how
...
maxy =  y_biof(2) + dy + Sxy.T_blayer;
pos_maxy = find((ySys >= maxy),1);
if isempty(pos_maxy)
    pos_maxy = length(ySys);
end

miny =  y_biof(1) - dy - Sxy.T_blayer;
