...
%                M = M*(bac_Ks(c(e),6)/(SLiqxy(O2_pos) + bac_Ks(c(e),6)));
                M = M*(bac_Ks(c(e),6)/(spcM(NO2_pos,rv(O2_pos)) + bac_Ks(c(e),6)));
            end
            mu_aux = mu_max*M;
            mu = mu_aux - maint;
            rg = mu*bac_m(c(e));
            ...
            if mu >= 0
                ...
                if act(c(e)) == 1
                    ...
                    NRVxy = zeros(size(sol_c));      %If bac is inactive: no substrate consumption, no change of mass
                    rg = zeros(size(rg));
                end
            end
