...
for i=2:(sDG0col - 1)
    Keq(:,i) = exp((DG0(:,i+1)+(i*1e10*(DG0(:,i+1) == 1e10))-(DG0(:,i)))/(-pOp.Rth*pOp.T)); %exp(-1e10) = 0 // if DG0 = 1e10 -> Keq = 0 //
end
...
%- Biofilm
xaux = (Sxy.dx/2):(2*bac.bac_rmax):(Sxy.maxxSys - Sxy.dx/2);
yaux = (Sxy.dy/2):(2*bac.bac_rmax):(bac.bac_ystart - Sxy.dy/2);
...
for i = 1:length(x_pn)
    ...
    %Random
%     SSaux = floor(rand*(St.numX-1))+1; %AMX
    SSaux = floor(rand*(St.numX-2))+1; %No AMX
    if SSaux == 3
%         SSaux = 4; %AMX
        SSaux = 2; %No AMX
    end
    if SSaux == 2
        SSaux = SSaux + double(rand<0.5); % Nitrobacter/NOB = Nitrospira/NOB = 50%
    end
