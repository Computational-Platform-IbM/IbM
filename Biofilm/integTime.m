...
R = boundary(R, dT_bac, 0, 0); %Reactor balances
...
[R, StVLiq, pH, ~, BBc, Lxy, Bce, Ve, Ve2, CC, DD] = DiffMatrices(0, 0, 0, 0, 0, R, 0, 0, 0, 0, 1);
...
%Effective diffusion due to porosity/tortuosity of biofilm |Df = (0,1]|
for p = 1: R.Sxy.nT
...
D = speye(R.Sxy.nT); %Identity matrix
Sxy = zeros((R.Sxy.ny-2), numStVLiq2);
aux = floor(((R.Sxy.nx-2)/2)-1)*(R.Sxy.ny-2);
for ii = 1:numStVLiq2
    S = StVLiq(ind(ii,1):ind(ii,2)); 
    Sxy(:,ii)  = S((aux+1):((R.Sxy.ny-2)+aux));
end
pHxy = pH((aux+1):((R.Sxy.ny-2)+aux));
                                       
out_integTime(0, R, Sxy, 0);

Fn = 2;F = ones(1, Fn); NNN = 0;
SS0 = 1e100*(2:-1:1);
Diff = R.kTr.Diffn*R.Sxy.dT/2; %Diffusion coefficient
...
while i < nTi
    ...
    parfor k = 1:numStVLiq2
        ...
        B3 = (2*Diff(k,1)*Bce)*U_aux(:,k);
        B4 = dT*(1000)*NRVliq_aux(:,k); %Reaction term
        B = B1 + B2 + B3 + B4;
        U_aux(:,k) = (D - Diff(k,1)*LL)\B;
        U_aux(:,k) = DD.*U_aux(:,k) + (1-DD).*Sbc_Dir_aux(:,k);
        RES(:, k) = ((abs((dx^2)*LL*U_aux(:,k)+(dx^2)*BBc*Sbc_Dir_aux(:,k)+(dx^2)*Bce*U_aux(:,k)+((dx^2)/Diffn(k,1))*(1000)*NRVliq_aux(:,k))/1000)./(1e-4 + U_aux(:,k)/1000));
    end
    for kn = 1:numStVLiq2
    ...
    if Time >= TimeSS   
        ...
        TolRES = TolRES*100; %perct (only NH3, NO2, NO3, O2) TolRES(1:4)
        ...
        if Time >= TimeBac 
            ...
            if Time >= TimeDiv
                ...
                [R, StVLiq, pH, Fb_layer, BBc, Lxy, Bce, Ve, Ve2, CC, DD] = DiffMatrices(BBc, Lxy, Bce, Ve, Ve2, R, StVLiq, pH, CC, DD, Fdiv);
...
if t == 0
    ...
    S = zeros(nySys-2,R.St.numStVLiq2);
    aux = round(((nySys-2)-(R.Sxy.ny-2))/2);
    for k = 1:R.St.numStVLiq2
        A = R.Sxy.Sbc_Dir(k)*ones(nySys-2,1);
        A(aux+1:aux+(R.Sxy.ny-2)) = Sxy(:,k);
        S(:, k) = A;
    end
    SSxy(:, :, 1) = S;
    ApH = R.pOp.pH*ones(nySys-2,1);
    ApH(aux+1:aux+(R.Sxy.ny-2)) = pHxy(:,1);
    pHs(:, 1) = ApH;
