    ...
    while sum(bac_m > bac.bac_mmax) >= 1 %|| sum(bac_m < bac.bac_mmin) >= 1
        for i=1:nold
            if bac_m(i-j) > bac.bac_mmax
                ...
                if bac_x(bac_n,1) < 0
                    bac_x(bac_n,1) = maxxSys - bac_r(bac_n,1);
                elseif bac_x(bac_n,1) > maxxSys
                    bac_x(bac_n,1) = bac_r(bac_n,1);
                end
                bac_s(bac_n,1) = bac_s(i-j);                %same cell type  
                bac_a(bac_n,1) = bac_a(i-j);
                act(bac_n,1) = 1;
%                 bac_mu_max(bac_n,1)  = bac_mu_max(i-j);     %same mu max
                bac_yield(bac_n,1) = bac_yield(i-j);        %same yield
...
if Fdiv == 1
    ...
    r = quadtree.pushing2D(length(bac_x), bac_x, bac_y, bac_r, 0.1, bac.s_dist, maxxSys);
    bac_x = r.bac_x;
    bac_y = r.bac_y;

    while sum(bac_y > bac.bac_ymax) > 0
        [~, I] = max(bac_y);
        bac_n = bac_n - 1;
