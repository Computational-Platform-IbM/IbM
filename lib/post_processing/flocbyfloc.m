...
%     floc2nBacs = determine_where_bacteria_in_flocs(1, grid, bac_saved);
    ...
    %% Write, plot or both?
    write_excel(save_times*constants.dT_save/(7*24), mass_matrix)
...
%% helper functions
function write_excel(time_write, mass_matrix)
    % Writing of results in Excel file
    path = 'RawData_flocbyfloc.xlsx';
    
    writematrix(time_write', path, 'Sheet', 'Time (weeks)')
...
function plot_mass_by_floc(time_plot, mass_matrix, nFlocs, nReplicates, nSpecies, colors_species)
