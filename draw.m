...
CM = [0 0 1;0 1 0;1 0 0]; %Array of colours for plotting
...
%     for i = 1:1:2
        ...
        clf;
        Time = B(1,1,i)/24;
        fprintf('>>>> Current draw time is %.1f days.',Time);
        bac_x = B(:,2,i);
        bac_y = B(:,3,i);
        bac_s = B(:,4,i);
        bac_r = B(:,5,i);
...
%             bacA = bac_a(k)/0.75;
            col = CM(bac_s(k),:);
            rectangle('Curvature',[1 1],'Position',[bac_x(k)-bac_r(k) bac_y(k)-bac_r(k) 2*bac_r(k) 2*bac_r(k)],'LineWidth',1,'FaceColor',(bacA> 0)*bacA*col,'EdgeColor',col*0.1);
...
%     axis([0 R.Sxy.maxx  0 R.Sxy.maxy]);
    ...
    xlabel('x')
    ylabel('y') 
    title({['Time = ',num2str(Time),' days. ','Number of bacteria = ',num2str(numBac)]})
%     title({[num2str(n_sp),' types of Bacteria',];['Time = ',num2str(Time),' hours. ','Number of bacteria = ',num2str(numBac)]})
    drawnow;
%     saveas(fB,['Time_',num2str(Time),'.jpg']);
    frame = getframe(fB);
...
%     Si = reshape(SSxy(:,j,:),R.Sxy.nySys-2,nT);
    ...
    S4 = reshape(SSxy(:,4,:),R.Sxy.nySys-2,nT);
%     S5 = reshape(SSxy(:,5,:),R.Sxy.nySys-2,nT);
%     S6 = reshape(SSxy(:,6,:),R.Sxy.nySys-2,nT);
%     S7 = reshape(SSxy(:,7,:),R.Sxy.nySys-2,nT);
%     Smax = max(R.Sxy.Sbc_Dir(j),max(max(Si)));
     Smax1 = max(max(max([S1;S2;S3;S4])));
     Smax2 = max(max(max(S4)));
    ...
    for i = 2:nT
        ...
        hold on  
        plot(minbac_x*1e6,R.Sxy.Sbc_Dir(1)*(1000), 'k*')
        hold on
        plot(maxbac_x*1e6,R.Sxy.Sbc_Dir(1)*(1000), 'k*')
        hold off      
...
%          pause
    ...
    writerObjS1 = VideoWriter('diffusion.avi');
    open(writerObjS1);
...
%         hold on
        ...
        plot(maxbac_x*1e6,R.Sxy.Sbc_Dir(1)*(1000), 'k*')
        hold off
         axis ([miniy*1e6 maxiy*1e6 0 Smax2])
%         axis ([miniy*1e6 maxiy*1e6 0 10^(-5)])
...
time = time/24;
    ...
    for i = 2:nT
        ...
        hold on
        plot(minbac_x*1e6,R.pOp.pH, 'k*')
        hold on
        plot(maxbac_x*1e6,R.pOp.pH, 'k*')
        hold off
...
function draw_substrate2(R,  All_StatesVar, B)
    time = All_StatesVar(:,1)/24;
    
    fprintf('\n>>> Evolution of Substrates and Products with time ... ')
    
    % Plot of liquid substrates
    fL = figure('Name','LIQUID Chemicals','Position',get(0, 'Screensize'),'Color','w');
    figure(fL)
    n_Liq = sum(strcmp(R.St.Phase,'L'));
    l = R.St.StNames(1:n_Liq,1);
    for i = 1: n_Liq
        plot(time, All_StatesVar(:,i+1).*1000, 'LineWidth',3)
        hold on
    end
    legend(l);
   ax = gca;
    ax.FontSize = 26;
    ax.LineWidth = 1;
    xlabel('Time (days)','FontSize',32) 
    ylabel('Concentration (mM)','FontSize',32) 
     saveas(fL,'SPLiquids.jpg')
    ...
    hold on
    plot(time, (R.Inf.St(1)+R.Inf.St(2)+R.Inf.St(3))*ones(size(time)), 'r');
    hold off
    legend(nt);

    n_Ph = sum(strcmp(R.St.Phase,'P'));n_Gas = sum(strcmp(R.St.Phase,'G'));

    % Plot of biomass
    colores = [0.6 0.2 1; 0 0.8 0;1 0.6 0;0 0 0.6;1 0 0.4];
    fX = figure('Name','Biomass overall','Position',get(0, 'Screensize'),'Color','w');
    figure(fX)
    n_X = sum(strcmp(R.St.Phase,'S'));
    x = R.St.StNames(n_Liq+n_Ph+n_Gas+1:n_Liq+n_Ph+n_Gas+n_X,1);
    for i = 1: n_X
        plot(time, All_StatesVar(:,n_Liq+n_Ph+n_Gas+i+1), 'Color',colores(i,:), 'LineWidth',3)
        hold on
    end
    legend(x);
       ax = gca;
    ...
    ax.LineWidth = 1;
     xlabel('Time (days)','FontSize',32) 
    ylabel('Concentration (M)','FontSize',32) 
     saveas(fX,'SPBiomass.jpg')
    ...
    for i = 1: n_X
        plot([0; time], ns(:,i), 'Color',colores(i,:))
        hold on
    end
    legend(x);
